...
<div id="body">
	<div id="header">
    	<div class="wrapper">
    		<a class="logo" >
    			<img width="229" height="43" alt="拉" src="style/images/logo.png">
    		</a>
...
<p>&nbsp;岗位要求： <br />${pif_Info.p_demand}</p>
 ...
 <%if(name==null){%>                   
                    <dd class="unresume">
	                        <div>
	                        	您还未登录，请先<a class="inline" href="#loginPop" title="登录">登录</a>
	                       	</div>
	                    </dd>
	                    <%}else{ %>
                    <dd class="resume resume_web">
				                        <div>
								          <span> 您的在线简历：<a title="简历名称" target="_blank" href="jianli.do?name=${sessionScope.username}"><strong>${stu_info.stu_jianli_title}</strong></a>(简历完整度:${stu_info.stu_resumeScore}%)</span><br>
								          <span>简历更新于${date}</span>
				                        </div>
...
<!-- 投递简历时  - 设置默认投递简历 -->
	<div style="height:280px;" class="popup" id="setResumeApply">
	    <table width="100%">
	    	<tbody><tr>
	    		<td class="f18 c5">确认投递简历：</td>
	    	</tr>
	    	<tr>
	        	<td>
                    <form class="resumeSetForm" id="resumeSendFormss" action="delivery.do?h_email=${hif_Info.h_email}&stu_id=${sessionScope.username}&p_id=${pif_Info.p_id}" method="post">
	            		<label class="radio">
	            			您的在线简历：
	            				            				<span title="jason的简历">${stu_info.stu_jianli_title}</span>
	            				            		</label>
            			<div class="setBtns">
            					            				
	            										<a target="_blank" href="showresumeView.do?stu_id=${sessionScope.username}">预览</a>|<a target="_blank" href="jianli.do?name=${sessionScope.username}">修改</a>
            			</div>
	            		<div class="clear"></div>
	            		<%-- <label class="radio">
	            			您的附件简历：
	            		...
	            		<div class="setBtns">
            					<a  href="jianli.do?name=${sessionScope.username}" target="_blank">上传附件简历(word、pdf)</a>
            			</div> --%>
            			<div class="clear"></div>
